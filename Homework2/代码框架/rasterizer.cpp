...
static bool insideTriangle(float x, float y, const Vector3f* _v)
{
    ...
    Eigen::Vector2f side_0, side_1, side_2;
    side_0 << _v[1].x() - _v[0].x(), _v[1].y() - _v[0].y();
    side_1 << _v[2].x() - _v[1].x(), _v[2].y() - _v[1].y();
    side_2 << _v[0].x() - _v[2].x(), _v[0].y() - _v[2].y();

    Eigen::Vector2f op0, op1, op2;
    op0 << x - _v[0].x(), y - _v[0].y();
    op1 << x - _v[1].x(), y - _v[1].y();
    op2 << x - _v[2].x(), y - _v[2].y();

    float bool0, bool1, bool2;
    bool0 = side_0.x() * op0.y() - side_0.y() * op0.x();
    bool1 = side_1.x() * op1.y() - side_1.y() * op1.x();
    bool2 = side_2.x() * op2.y() - side_2.y() * op2.x();
    if ((bool0 > 0 && bool1 > 0 && bool2 > 0) || (bool0 < 0 && bool1 < 0 && bool2 < 0)) {
        return true;
    }
    else return false;
}
        ...
        for (int y = min_y; y <= max_y; y++) {
            ...
            for (int i = 0; i < 4; i++) {
                if (insideTriangle(x + sample_points[i][0], y + sample_points[i][1], t.v)) {
                    auto [alpha, beta, gamma] = computeBarycentric2D(x + sample_points[i][0], y + sample_points[i][1], t.v);
